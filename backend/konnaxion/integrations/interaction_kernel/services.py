...
def decision_artifact_payload(record: DecisionRecord) -> dict[str, Any]:
    return {
        ...
        "decision": {
            "id": str(record.pk), "topic": record.topic_id, "protocol": record.protocol_id,
            "title": record.title, "description": record.description, "status": DecisionRecord.STATUS_PUBLISHED,
            "baseline_result": record.baseline_result_json or {},
            "reading_result_refs": list(record.reading_result_refs or []),
            "opened_at": _iso(record.opened_at), "closed_at": _iso(record.closed_at), "published_at": _iso(record.published_at),
        },
    }
...
def publish_decision_record(*, decision_id: int) -> DecisionRecord:
    with transaction.atomic():
        ...
        record.save(update_fields=["status", "published_at", "published_payload", "artifact_digest", "updated_at"])
        return record
...
def enqueue_decision_execution(*, decision_id: int, target_organization: str, target_world: str | None = None, execution_scope: Mapping[str, Any] | None = None) -> InteractionEmission:
    with transaction.atomic():
        ...
        envelope = build_decision_execute_envelope(
            ...
            target_world=target_world, effective_at=_iso(record.published_at), execution_scope=execution_scope,
        )
        ...
        if existing is not None:
            if existing.request_fingerprint != fingerprint:
                raise IKIdempotencyConflict("Same IK idempotency key already exists with divergent semantic content.")
            return existing
        emission = InteractionEmission.objects.create(
            interaction_id=envelope["id"], profile_id=envelope["profile"]["id"], profile_version=envelope["profile"]["version"],
            target_system=envelope["target"]["system"], target_organization=envelope["target"].get("organization") or "",
            target_world=envelope["target"].get("world") or "", subject_type=envelope["subject"]["type"],
...
def build_konnaxion_export(*, decision: DecisionRecord) -> dict[str, Any]:
    ...
    return {
        ...
        "source_revision": str(decision.revision), "scope": scope, "subjects": [{"type": "decision", "id": str(decision.pk)}],
        "items": [{"type": "canonical_decision_artifact", "ref": decision.artifact_id, "digest": f"sha256:{decision.artifact_digest}"}],
        "intended_use": ["kristal_compilation"], "provenance": scope,
        "integrity": {"algorithm": "sha256", "digest": sha256_jcs(decision.published_payload)},
    }
