...
def reconcile_vote_schema(apps, schema_editor):
    with schema_editor.connection.cursor() as cursor:
        ...
        # ------------------------------------------------------------------
        cursor.execute(
            f"ALTER TABLE {SCHEMA}.vote_modality "
            "ADD COLUMN IF NOT EXISTS id integer"
        )

        cursor.execute(
            f"CREATE SEQUENCE IF NOT EXISTS {SCHEMA}.vote_modality_id_seq"
        )

        cursor.execute(
            f"ALTER SEQUENCE {SCHEMA}.vote_modality_id_seq "
            f"OWNED BY {SCHEMA}.vote_modality.id"
        )

        cursor.execute(
            f"ALTER TABLE {SCHEMA}.vote_modality "
            f"ALTER COLUMN id SET DEFAULT "
            f"nextval('{SCHEMA}.vote_modality_id_seq')"
        )

        cursor.execute(
            f"UPDATE {SCHEMA}.vote_modality "
            f"SET id = nextval('{SCHEMA}.vote_modality_id_seq') "
            "WHERE id IS NULL"
        )
        ...
        for name in (
            ...
            cursor.execute(
                f"""
                INSERT INTO {SCHEMA}.vote_modality (name, parameters)
                VALUES (%s, '{{}}'::jsonb)
        ...
        if max_id > 0:
            cursor.execute(
                f"SELECT setval("
                f"'{SCHEMA}.vote_modality_id_seq', %s, true"
                f")",
                (max_id,),
            )
        ...
        # ------------------------------------------------------------------
        if not _column_exists(cursor, "vote", "modality_id"):
            cursor.execute(
                f"ALTER TABLE {SCHEMA}.vote "
                "ADD COLUMN modality_id integer"
            )

        if _column_exists(cursor, "vote", "modality_name"):
            cursor.execute(
                f"""
                UPDATE {SCHEMA}.vote AS vote
                SET modality_id = modality.id
                FROM {SCHEMA}.vote_modality AS modality
        ...
        if not _constraint_exists(
            ...
            cursor.execute(
                ...
                f"REFERENCES {SCHEMA}.vote_modality(id) "
                "ON DELETE RESTRICT"
            )
