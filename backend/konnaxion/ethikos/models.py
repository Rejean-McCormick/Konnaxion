...
class DecisionRecord(models.Model):
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=(
                    ~models.Q(status__in=("closed", "published"))
...
class InteractionEmission(models.Model):
    """Konnaxion-owned durable outbound IK delivery intent.
...
class OrgoImpactPublication(models.Model):
    ...
    Rows live in Konnaxion's ethiKos schema. Orgo identifiers are references
    only; they are never foreign keys into the Orgo database.
    """
