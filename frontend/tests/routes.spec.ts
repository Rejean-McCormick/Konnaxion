...
test.describe('Smoke all pages', () => {
  for (const p of routes) {
    test(`GET ${p}`, async ({ page }) => {
      ...
      page.on('pageerror', (e) =>
        logs.push({ type: 'pageerror', text: String(e) }),
      );
      ...
      try {
        resp = await page.goto(`http://localhost:3000${p}`, {
          waitUntil: 'domcontentloaded',
          timeout: NAV_TMO,
        });
      } catch (e: any) {
        const { message } = normalizeError(e); // statusCode removed to avoid unused var
      ...
      const severe = logs.filter(
        (l) =>
          ['error', 'assert', 'pageerror'].includes(l.type) &&
          !/favicon\.ico/i.test(l.text),
      );
