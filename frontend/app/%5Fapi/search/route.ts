...
const WORLD_KEY_RE = /^[a-z0-9](?:[a-z0-9-]{0,118}[a-z0-9])?$/i
...
async function searchKnowledge(
  ...
  for (const endpoint of KNOWLEDGE_SEARCH_ENDPOINTS) {
    const scopedEndpoint = worldKey ? `/w/${worldKey}${endpoint}` : endpoint
    const url = new URL(buildBackendUrl(scopedEndpoint, request))
...
export async function GET(request: NextRequest) {
  ...
  const rawWorld = request.nextUrl.searchParams.get('world')?.trim().toLowerCase() ?? ''
  const worldKey = rawWorld && WORLD_KEY_RE.test(rawWorld) ? rawWorld : null

  if (rawWorld && !worldKey) {
    return NextResponse.json(
      { error: 'Invalid World key' },
