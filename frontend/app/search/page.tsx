...
export default function GlobalSearchPage() {
  ...
  const runSearch = useCallback(
    async (raw: string) => {
      ...
      try {
        ...
        // Keep URL in sync (so /search?q=… is shareable and reloadable)
  ...
  const { routeResults, knowledgeResults, otherResults } = useMemo(() => {
    ...
    const knowledge = results.filter((r) => r.id.startsWith('knowledge:'));
    const other = results.filter(
      (r) => !r.id.startsWith('route:') && !r.id.startsWith('knowledge:'),
    );
  ...
  return (
    <MainLayout>
      <div className="container mx-auto p-5" style={{ maxWidth: 1200 }}>
        {/* Page header – aligned with module shells (container + h1 + description) */}
        <div className="mb-4">
        ...
        <Card className="mb-4">
          ...
          {!hasSearched && !error && (
            <Paragraph type="secondary" style={{ marginTop: 12 }}>
              Tip: start typing a module name (EkoH, ethiKos, keenKonnect, KonnectED,
              Kreative) or a topic to jump directly to a page or resource.
            </Paragraph>
          )}
        </Card>
