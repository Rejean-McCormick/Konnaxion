...
const outputDir = path.join(tourDir, 'screenshots-fullpage')
...
async function main() {
  console.log('[SCRIPT] FULLPAGE_V3_NO_HOME_TEXT')
  console.log('[MODE] fullPage=true | accueil non bloquant')
  await checkApp()
  ...
  try {
    // 1 — accueil Konnaxion : aucune dépendance au texte "Welcome to Konnaxion".
    await page.goto(`${baseURL}/`, { waitUntil: 'domcontentloaded', timeout: 45_000 })
    await page.locator('body').waitFor({ state: 'visible', timeout: 20_000 })
    await page.waitForLoadState('networkidle', { timeout: 5_000 }).catch(() => {})
    await shot(page, 'konnaxion-home', 'Konnaxion — accueil', null)

    // 2 — liste des sujets Ethikos
    await page.goto(`${baseURL}/ethikos/deliberate/elite?sidebar=ethikos`, {
    ...
    // 8 — Pulse
    await page.goto(`${baseURL}/ethikos/pulse/overview?sidebar=ethikos`, {
      waitUntil: 'domcontentloaded',
      timeout: 45_000,
    })
      ...
      JSON.stringify(
        {
          ...
          script_version: 'FULLPAGE_V3_NO_HOME_TEXT',
